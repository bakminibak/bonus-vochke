...
export const ContinueGame = ({ sessionState, isLoading, updateLevel }) => {
  ...
  const msg_EndSession =
    "Žao nam je, blaga više nema. Prošlo je 2 sata od početka tvoje ekspedicije. Pirati su stigli pre tebe i uzeli sve. \n  Vrati se nazad na svoj Xtip nalog i oprobaj sreću u nekoj drugoj avanturi.";
  const msg_RepeatLanding =
    "Žao nam je, već si jednom bio na ovoj  ekspediciji i pokupio sva blaga. \n Vrati se nazad na svoj Xtip nalog i oprobaj sreću u nekoj drugoj avanturi.";
  const msg_ContinuePlay =
    "Dobro je da si se vratio, skriveno blago te i dalje čeka! \n Do sada si osvojio {000%} bonusa. Nastavi tamo gde si stao.";
  ...
  console.log("sessionState", sessionState.gameState.level);
    ...
    if (Object.keys(sessionState).length > 0) {
      ...
      if (sessionState.finished === false) {
        ...
        if (
          ...
          sessionState.gameState.level < 3 &&
          sessionState.gameState.totalPoints
        ) {
          let newMsgPoints = String(
            msg_ContinuePlay.replace(
              "{000%}",
              sessionState.gameState.totalPoints + "%"
            )
          );
  ...
  return (
    <div className="continue-game" ref={animationContainer}>
      ...
      {/* {showBtn && ( */}
        <div className={"btn continue_btn "}>
          {" "}
          <img src="./images/btns/NASTAVI.png" alt="Restart Game" onClick={restartGame} />
        </div>
      {/* )} */}
    </div>
  );
