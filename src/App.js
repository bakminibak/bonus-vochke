...
const gameStateInitSate = {
  finished: false,
  gameState: { level: 1, totalPoints: 0, opened: false },
};
...
function App() {
  ...
  const [currentSession, setCurrentSession] = useState(gameStateInitSate);
  ...
  const createSession = () => async (requestData) => {
    ...
    //     !data.didWin &&
    //     data.gameState !== null &&
    //     data.gameState.opened.length > 0
    //   ) {
  ...
  const bonusMasterOpen = (params) => async (params) => {
    ...
    setIsLoaded(false);
    // const http = getAxiosInstance();
    // let _levelPrizes = [100, -50, 30];
    setLevelPrizes([100, -50, 30]);
    ...
    <div className={`App ${orientation}`}>
      ...
      <main>
        ...
        <MobileView className="mobile-view">
          ...
          {/* {!gameRestarted && isSessionActive && currentLevel === -2 && (
            <Login
              updateLevel={updateLevel}
              login={login()}
              register={register()}
          ...
          {!gameRestarted && isSessionActive && currentLevel === -1 && (
            <WelcomeDeskMobile
              isLoading={() => {
                setIsLoading();
              }}
          ...
          {!gameRestarted && isSessionActive && currentLevel === 0 && (
          ...
          {!gameRestarted && isSessionActive && currentLevel === 1 && (
            <LevelMobile
              ...
              currentLevel={currentLevel}
              currentSession={currentSession}
              bonusMasterOpen={bonusMasterOpen()}
              bonusMasterTransfer={bonusMasterTransfer()}
              handleNextLevel={() => {
                loadNextLevel();
              }}
            />
          ...
          {!gameRestarted && isSessionActive && currentLevel === 3 && (
            <LevelMobile
              isLoading={() => {
                setIsLoading();
              }}
          ...
          {!gameRestarted &&
            isSessionActive &&
            currentLevel > 0 &&
            currentLevel < 4 && <LevelPoints totalPoints={totalPoints} />}
          {!gameRestarted && isSessionActive && currentLevel > 3 && (
            <EndScrMobile
